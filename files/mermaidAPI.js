 ...
 */
var Logger = require('$:/plugins/michaeljmcd/mermaid/logger.js');
 ...
 */
_exports.version = function(){
    return require('../package.json').version;
};

_exports.encodeEntities = function(text){
    var txt = text;
...
_exports.decodeEntities = function(text){
    var txt = text;
...
var render = function(id, txt, cb, container){
    ...
    window.txt = txt;
    txt = _exports.encodeEntities(txt);
    ...
    switch(graphType){
        ...
        case 'info':
            ...
            info.draw(txt,id,_exports.version());
            if(config.cloneCssStyles) {
                utils.cloneCssStyles(element.firstChild, []);
            }
            break;
    ...
    svgCode = _exports.decodeEntities(svgCode);
...
_exports.render = function (id, text, cb, containerElement) {
    try{
...
_exports.initialize = function(options){
...
_exports.getConfig = function(){
    return config;
};

_exports.parseError = function(err, hash) {
